...
bool Level::IsFireball(int x, int y)
{
    return m_pLevelData[GetIndexFromCoordinates(x, y)] == 155;
}
...
bool Level::Convert(int* playerX, int* playerY)
{
    ...
    for (int y = 0; y < m_height; ++y)
    {
        for (int x = 0; x < m_width; ++x)
        {
            ...
            case '$':
                m_pLevelData[index] = ' ';
                m_pActors.push_back(new Money(x, y, 1));
                break;
            case 'F':
                m_pLevelData[index] = ' ';
                m_pActors.push_back(new FireballPickup(x, y));
                break;
            case '@':
                m_pLevelData[index] = ' ';
